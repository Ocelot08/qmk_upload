...
void readSensor(void) {
	...
	// delta yh,yl can be swapped with xh,xl to change direction 90 degrees 
	// note report.x/y may also be needed for correct orientation
    delta_x += convertDeltaToInt(burst_data[delta_y_h], burst_data[delta_y_l]);
    delta_y -= convertDeltaToInt(burst_data[delta_x_h], burst_data[delta_x_l]);
...
void pointing_device_task(void) {
    ...
    if(motion_ind) {
        ...
        // report.x flips the cursor axis 
		report.x = -report.x;
        report.y = delta_y < -127 ? -127 : delta_y > 127 ? 127 : delta_y;
		// report.y flips the cursor axis
		report.y = -report.y;
